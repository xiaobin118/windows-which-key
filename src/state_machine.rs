...
impl StateMachine {
    ...
    pub fn handle_event(&mut self, event: KeyEvent) -> Option<UiCommand> {
        match event {
            ...
            // 控制面板不属于浮层状态机，由主循环直接处理
            KeyEvent::ToggleControlPanel => None,
        }
    }
    ...
    fn handle_modifier_down(&mut self, modifier: Modifier) -> Option<UiCommand> {
        ...
        match &self.state {
            ...
            // 尚未显示，等 tick 用完整的修饰键组合触发首查
            State::Waiting { .. } => None,
            State::Showing { path } => self.refreshed_root_entries(path),
        }
    }
    ...
    fn handle_modifier_up(&mut self, modifier: Modifier) -> Option<UiCommand> {
        ...
        if self.pressed_modifiers.is_empty() {
            self.state = State::Idle;
            Some(UiCommand::Hide)
        } else if let State::Showing { path } = &self.state {
            self.refreshed_root_entries(path)
        } else {
            None
        }
    }

    /// 根层级的条目按“当前按住的修饰键”精确匹配过滤，因此修饰键组合
    /// 变化（追加或松开其一）时要重查并刷新面板；分组内部的条目
    /// 不按修饰键过滤，保持原样。
    fn refreshed_root_entries(&self, path: &[ShortcutKey]) -> Option<UiCommand> {
        if !path.is_empty() {
            return None;
        }
...
mod tests {
    ...
    fn test_multiple_modifiers() {
        ...
        // Press another modifier - root entries refresh to the wider chord
        match sm.handle_event(KeyEvent::ModifierDown(Modifier::Shift)) {
            Some(UiCommand::UpdateEntries { entries, .. }) => {
                assert_eq!(entries.len(), 1);
                assert_eq!(entries[0].key, "C-S-p");
            }
            command => panic!("expected refreshed Ctrl+Shift entries, got {command:?}"),
        }
        assert_eq!(sm.pressed_modifiers.len(), 2);
        assert!(matches!(sm.state, State::Showing { .. }));

        // Release one modifier - root entries refresh back to Ctrl-only
        match sm.handle_event(KeyEvent::ModifierUp(Modifier::Shift)) {
            Some(UiCommand::UpdateEntries { entries, .. }) => {
                assert_eq!(entries.len(), 1);
                assert_eq!(entries[0].key, "C-c");
            }
            command => panic!("expected refreshed Ctrl-only entries, got {command:?}"),
        }
        assert!(matches!(sm.state, State::Showing { .. }));
    ...
    #[test]
    fn pressing_shift_after_ctrl_showing_refreshes_to_ctrl_shift() {
        let mut sm = fixture_state_machine();
        ...
        // 300ms 延迟已过、面板正显示 C-xxx 时再按 Shift：应刷新为 C-S-xxx
        match sm.handle_event(KeyEvent::ModifierDown(Modifier::Shift)) {
            Some(UiCommand::UpdateEntries {
                entries,
                breadcrumb,
    ...
    #[test]
    fn modifier_change_inside_a_group_does_not_refresh_entries() {
        let mut sm = fixture_state_machine();
        sm.show_immediately_for_test(ModifierSet::empty());

        assert!(matches!(
            sm.handle_event(KeyEvent::KeyDown(Key::G)),
            Some(UiCommand::UpdateEntries { .. })
        ));

        assert!(sm
            .handle_event(KeyEvent::ModifierDown(Modifier::Ctrl))
            .is_none());
        assert!(matches!(sm.state, State::Showing { path } if path.len() == 1));
    }

    #[test]
    fn second_modifier_while_waiting_keeps_waiting_without_refresh() {
        let mut sm = fixture_state_machine();

        sm.handle_event(KeyEvent::ModifierDown(Modifier::Ctrl));
        assert!(sm
            .handle_event(KeyEvent::ModifierDown(Modifier::Shift))
            .is_none());
        assert!(matches!(sm.state, State::Waiting { .. }));
        ...
        match sm.tick() {
            ...
            command => panic!("expected Ctrl+Shift entries after waiting, got {command:?}"),
        }
    }
