...
/// 窗口实际所在显示器的 DPI 缩放系数（相对 96）。
/// 注意：Per-Monitor V2 进程里 GetDpiForSystem 恒返回 96，
/// 必须用 GetDpiForWindow 才能拿到真实的显示器缩放。
pub fn dpi_factor_for(hwnd: HWND) -> f64 {
...
/// 仅供测试与无窗口上下文使用的粗略缩放（PMv2 下恒为 1.0）。
pub fn scale_factor() -> f64 {
    let dpi = unsafe { GetDpiForSystem() };
    dpi as f64 / 96.0
}

/// 把逻辑像素换算成当前系统 DPI 下的物理像素。进程声明了
/// Per-Monitor V2 感知后，窗口尺寸必须用物理像素。
pub fn scale_logical_for(hwnd: HWND, logical: i32) -> i32 {
    (logical as f64 * dpi_factor_for(hwnd)).round() as i32
}
