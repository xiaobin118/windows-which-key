...
impl WebView2Bridge {
    ...
    /// `width`/`height` 为宿主窗口的物理像素尺寸，WebView bounds 与之保持一致。
    pub fn new(hwnd: HWND, width: i32, height: i32) -> Result<Self> {
        unsafe {
    ...
    /// 调整 WebView2 的 bounds（物理像素），与宿主窗口保持一致。
    pub fn resize(&self, width: i32, height: i32) -> Result<()> {
        unsafe {
            self.controller
                ...
                .context("调整 WebView2 bounds 失败")?;
        }
        Ok(())
    }

    /// 在页面里执行任意 JS（send_command 的底层原语）。
    pub fn execute_script(&self, js: &str) -> Result<()> {
        let js_str: windows::core::HSTRING = js.into();
        unsafe {
            self.webview
    ...
    /// 把页面 `window.chrome.webview.postMessage(...)` 的消息以 JSON
    /// 字符串转发到指定通道，实现 JS→Rust 通信。
    pub fn forward_web_messages(&self, sender: Sender<String>) -> Result<()> {
        unsafe {
    ...
    pub fn send_command(&self, cmd: &UiCommand) -> Result<()> {
        ...
        self.execute_script(&format!("window.postMessage({json_msg}, '*');"))
    }
}
...
fn command_json(cmd: &UiCommand) -> serde_json::Value {
    match cmd {
        ...
        UiCommand::ApplyTheme { theme } => json!({
            "type": "theme",
            "theme": theme,
        }),
